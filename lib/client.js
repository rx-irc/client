...
module.exports = class Client {
	 ...
	 * @param {boolean} [options.stdin=true] Pipe stdin into the socket if available.
	 */
	constructor(options) {
	...
	connect(callback) {
		...
		if (this.settings.stdin && process.stdin.isTTY) {
...
*/
	...
	prepareSubscriptions(callback) {
		this.messageIn$.subscribe(message => {
			switch (message.command) {
				...
				case 'ERR_NICKNAMEINUSE': {
					logger.error('S: Nickname in use');
					this.settings.nick = this.settings.nick + '_';
					this.rawOut$.next(`NICK ${this.settings.nick}`);
					break;
				}
		...
		// KICK
		this.kick$.subscribe(({ who, by, channel, reason }) => {
			logger.log(`S: KICK ${who} by ${by} from ${channel} :${reason}`);

			// If the bot was kicked, remove the channel from the store.
			if (who === this.store.get('user.nick')) {
				this.store.remove('channels', ['name', channel]);
			// Otherwise, remove the user from the channel.
			} else {
				let chan = this.store.find('channels', ['name', channel]);
